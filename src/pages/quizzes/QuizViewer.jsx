      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden mb-6 sm:mb-8">
            {/* Banner */}
            <div className={`bg-gradient-to-r ${quizState.result.passed ? 'from-green-500 to-emerald-600' : 'from-orange-500 to-red-500'} px-4 sm:px-8 py-5 sm:py-6`}>
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
                ...
                <div className="text-center sm:text-right">
                  <p className="text-4xl sm:text-5xl font-bold text-white">{quizState.result.score}%</p>
            ...
            <div className="p-4 sm:p-8">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 sm:p-6 border border-green-100 text-center">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-lg">
                    <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                  <p className="text-3xl sm:text-4xl font-bold text-green-600 mb-1">{quizState.result.correct}</p>
                  <p className="text-green-700 font-medium text-sm sm:text-base">Correct Answers</p>
                </div>
                <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-xl p-4 sm:p-6 border border-red-100 text-center">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-red-500 to-orange-500 rounded-xl flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-lg">
                    <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <p className="text-red-700 font-medium text-sm sm:text-base">Incorrect Answers</p>
                </div>
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 sm:p-6 border border-blue-100 text-center">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center mx-auto mb-2 sm:mb-3 shadow-lg">
                    <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                    </svg>
                  </div>
                  <p className="text-3xl sm:text-4xl font-bold text-blue-600 mb-1">{quizState.result.score}%</p>
                  <p className="text-blue-700 font-medium text-sm sm:text-base">Total Score</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            <div className="px-4 sm:px-8 py-4 sm:py-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-indigo-50">
            ...
            <div className="p-4 sm:p-8 space-y-4 sm:space-y-6">
              {quizToShow.questions.map((question, index) => {
                ...
                if (question.type === 'true_false') {
                  const studentValue = detail?.submitted_answer;
                  const correctValue = detail?.correct_answer ?? question.correct_answer;
                  studentAnswerText = studentValue === 'true' ? 'True' : studentValue === 'false' ? 'False' : null;
                  correctAnswerText = correctValue === 'true' ? 'True' : correctValue === 'false' ? 'False' : null;
                } else {
                  const studentAnswerObj = question.answers.find((a) => a.id === detail?.submitted_answer_id);
                  const correctAnswerObj = question.answers.find((a) => a.id === detail?.correct_answer_id);
                  studentAnswerText = studentAnswerObj?.answer_text ?? null;
                  correctAnswerText = correctAnswerObj?.answer_text ?? null;
                }
                ...
                return (
                  ...
                  >
                    {/* Question Header */}
                    <div className="px-4 sm:px-6 py-3 sm:py-4 flex items-start gap-3 sm:gap-4 bg-gray-50">
                      <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${isCorrect
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-1">
                          <span className="text-xs sm:text-sm font-semibold text-gray-500">Question {index + 1}</span>
                          <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-bold ${isCorrect
                        ...
                        <p className="text-gray-900 font-medium text-sm sm:text-base break-words">{question.question_text}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 sm:p-6 space-y-2">
                      {question.type === 'true_false' ? (
                        ['true', 'false'].map((value) => {
                          ...
                          const isCorrectAnswer = (detail?.correct_answer ?? question.correct_answer) === value;
                          return (
                            <div
                              key={value}
                              className={`flex items-center gap-3 p-3 rounded border text-sm sm:text-base ${isCorrectAnswer
                            ...
                            >
                              <span className={`w-4 h-4 flex-shrink-0 flex items-center justify-center ${isCorrectAnswer ? 'text-green-600' : isStudentAnswer ? 'text-white' : 'text-gray-400'
                                }`}>
                                {isCorrectAnswer || isStudentAnswer ? '✓' : '○'}
                              </span>
                              <span className={`break-words ${isCorrectAnswer ? 'font-medium' : ''}`}>
          ...
          {/* Back Button */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6 flex justify-center mt-6 sm:mt-7">
            <button
              onClick={() => navigate(-1)}
              className="inline-flex items-center gap-2 bg-gray-100 text-gray-700 px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg hover:bg-gray-200 transition-colors font-medium text-sm sm:text-base"
            >
