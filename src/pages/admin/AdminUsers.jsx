...
export default function AdminUsers() {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* Toast */}
      {toast && (
        <div className="fixed top-5 right-5 z-50 bg-[#1e2534] border border-white/10 text-white px-5 py-3 rounded-xl shadow-2xl text-sm animate-in fade-in slide-in-from-top-2 duration-200">
          {toast}
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Manage all students and instructors on your platform.</p>
        </div>
        <span className="text-gray-400 text-sm bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
          {meta.total ?? 0} users
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <input
          ...
          className="flex-1 min-w-[200px] bg-[#151922] border border-white/10 text-white text-sm rounded-xl px-4 py-2.5 outline-none focus:border-violet-500/50 placeholder-gray-500 transition-colors"
        />
        <select
          value={roleFilter}
          onChange={e => { setRole(e.target.value); setPage(1); }}
          className="bg-[#151922] border border-white/10 text-gray-300 text-sm rounded-xl px-4 py-2.5 outline-none focus:border-violet-500/50 transition-colors cursor-pointer"
        >
              ...
              <tbody className="divide-y divide-white/5">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-5 py-4 text-gray-300 text-sm">{u.enrollments_count ?? 0}</td>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2 text-sm bg-[#151922] border border-white/10 text-gray-300 rounded-xl hover:bg-white/5 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            >
              Previous
            </button>
            <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#151922] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
            ...
            {['name', 'email'].map(field => (
              <div key={field}>
                ...
                <input
                  ...
                  className="w-full bg-[#0f1117] border border-white/10 text-white text-sm rounded-xl px-4 py-2.5 outline-none focus:border-violet-500/50 transition-colors"
                />
              </div>
            ))}
            <div>
              <label className="block text-gray-400 text-xs font-medium mb-1.5">Role</label>
              <select value={editForm.role} onChange={e => setEditForm(f => ({ ...f, role: e.target.value }))} className="w-full bg-[#0f1117] border border-white/10 text-gray-300 text-sm rounded-xl px-4 py-2.5 outline-none focus:border-violet-500/50 transition-colors cursor-pointer">
                <option value="student">Student</option>
                <option value="instructor">Instructor</option>
              </select>
            </div>
            <div>
              ...
              <select value={editForm.status} onChange={e => setEditForm(f => ({ ...f, status: e.target.value }))} className="w-full bg-[#0f1117] border border-white/10 text-gray-300 text-sm rounded-xl px-4 py-2.5 outline-none focus:border-violet-500/50 transition-colors cursor-pointer">
                <option value="active">Active</option>
                <option value="suspended">Suspended</option>
                <option value="banned">Banned</option>
              </select>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setEditUser(null)} className="flex-1 py-2.5 text-sm bg-white/5 text-gray-300 rounded-xl hover:bg-white/10 transition-colors border border-white/10">Cancel</button>
              <button onClick={saveEdit} disabled={saving} className="flex-1 py-2.5 text-sm bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-xl hover:from-violet-700 hover:to-indigo-700 transition-all disabled:opacity-50">
